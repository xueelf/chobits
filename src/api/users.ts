...
export interface SendUsersMessagePayload {
  /** 文本内容 */
  content: string;
  /** 消息类型 */
  msg_type: MsgType;
...
export interface SendUserFilePayload {
  /** 媒体类型 */
  file_type: FileType;
  /** 需要发送媒体资源的 url */
  url: string;
...
export default (request: TotteInstance) => {
  return {
    ...
    sendUserMessage(
      openid: string,
      payload: SendUsersMessagePayload,
    ): Promise<Result<MessageResponse>> {
      return request.post(`/v2/users/${openid}/messages`, payload);
    },
     ...
     * 发送富媒体消息到用户。
     */
    sendUserFile(openid: string, payload: SendUserFilePayload): Promise<Result<UserFile>> {
      return request.post(`/v2/users/${openid}/files`, payload);
    },
  };
};
