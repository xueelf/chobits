...
/** 已发送的流式单聊消息信息。 */
export interface SendUserStreamMessage {
...
/** 单聊富媒体分片上传信息。 */
export interface PrepareUserFileUpload {
...
export default (request: TotteInstance) => {
  return {
    ...
    sendUserMessage(user_openid: string, message: SendUserMessagePayload) {
      return request.post<SendUserMessage>(`/v2/users/${user_openid}/messages`, message);
    },
     ...
     * @throws 50002 频率限制
     */
    sendUserStreamMessage(user_openid: string, message: SendUserStreamMessagePayload) {
      return request.post<SendUserStreamMessage>(`/v2/users/${user_openid}/stream_messages`, message);
    },
     ...
     */
    recallUserMessage(user_openid: string, message_id: string) {
     ...
     */
    uploadUserFile(user_openid: string, file: UploadUserFilePayload) {
      return request.post<UploadUserFile>(`/v2/users/${user_openid}/files`, file);
    },
     ...
     * @throws 40093001 文件上传失败，请重试
     */
    prepareUserFileUpload(user_id: string, file: PrepareUserFileUploadPayload) {
      return request.post<PrepareUserFileUpload>(`/v2/users/${user_id}/upload_prepare`, file);
    },
     ...
     * @throws 40093002 超过今天发送文件容量上限
     */
    finishUserFileUploadPart(user_id: string, part: FinishUserFileUploadPartPayload) {
